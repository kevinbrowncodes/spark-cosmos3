...
- [ ] `capabilities.agent` is declared by the sidecar with the count range, default confirm mode and the run fields, exactly as flow `PROTOCOL.md` specifies after STORY-601
- [ ] The sidecar serves the protocol-shaped `/flow/agent/*` routes as a **mirror of `/agent/*`** — same handlers, no second implementation; the CLI keeps working unchanged
...
- [ ] `gateway/server.py`, `flow/runs.py` and the executor untouched — the UI is a client of what STORY_030 built
- [ ] `flow/` stays ≥ 95 % line coverage
