...
if curl -sf -m 3 -o /dev/null "http://localhost:11434/api/version" && docker compose exec -T flow python -c "import httpx; httpx.get('http://host.docker.internal:11434/api/version', timeout=3)" >/dev/null 2>&1; then
